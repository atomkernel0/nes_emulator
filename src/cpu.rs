...
impl CPU {
    ...
    pub fn get_absolute_address(&mut self, mode: &AddressingMode, addr: u16) -> u16 {
        match mode {
            ...
            AddressingMode::Indirect_Y => {
                ...
                if self.page_crossed(deref_base, deref) {
                    self.cycles += 1;
                }
                deref
            }
    ...
    pub fn get_absolute_address_readonly(&self, mode: &AddressingMode, addr: u16) -> u16 {
        match mode {
            AddressingMode::Immediate => addr,
            ...
            AddressingMode::Absolute_X => {
                let base = self.mem_read_u16(addr);
                let addr = base.wrapping_add(self.register_x as u16);
                // Pas de modification des cycles dans la version readonly
                addr
            }
            AddressingMode::Absolute_Y => {
                let base = self.mem_read_u16(addr);
    ...
    /* NES CPU does not use demical mode */
